...
export const getPricing = () => async (dispatch) => {
  try {
    const res = await axios.get(
      "http://localhost:5000/api/v1/client/getallpricing"
    );
    console.log(res);
    dispatch({
      type: GET_PRICING,
...
export const getPackageprice = () => async (dispatch) => {
  try {
    const res = await axios.post(
      "http://localhost:5000/api/v1/client/packageprice"
    );
    console.log(res);
    dispatch({
      type: GET_PACKAGEPRICE,
      payload: res.data.packageprice,
    });
  } catch (err) {
    console.log(err);
  }
